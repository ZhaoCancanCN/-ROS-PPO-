...
class StageWorld():
    def __init__(self, beam_num, index, num_env):
        ...
        self.map_size = np.array([8., 8.], dtype=np.float32)  # 20x20m  #这个是作者用的   我用的地图不是方的  这个其实作者用的长宽为40成40的  也没看到在那用阿
        self.goal_size = 0.5
    ...
    def ground_truth_callback(self, GT_odometry):
        ...
        v = np.sqrt(v_x**2 + v_y**2)   #获取的速度是(v  odometry .z)
    ...
    def get_local_goal(self):   #这个获取的就算相对位置
